...
func encodeUpdateActivityResponse(w http.ResponseWriter, status int, message string, update_code string ) {

   logger.Log(fmt.Sprintf("encodeUpdateActivityResponse status: %d (%s)", status, message))
   jsonAttributes(w)
   w.WriteHeader(status)
   if err := json.NewEncoder(w).Encode(api.UpdateActivityResponse{Status: status, Message: message, UpdateCode: update_code}); err != nil {
      log.Fatal(err)
   }
}
